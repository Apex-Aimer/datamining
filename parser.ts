...
function decompressPakfileInit() {
    let input_byte_pos_init: number;
    let byte_init: number;
    let decompressed_size_bits: number;
    let byte_1_low: number;
    let  input_byte_pos_1: number; // r10
	let  bit_pos_final: number; // ebp
	let  byte_1: number; // rdi
	let  brih_bits: number; // er11
	let  inv_mask_in: number; // r8
	let  byte_final_full: number; // rbx
	let  bit_pos_final_1: number; // rax
	let byte_bit_offset_final: number; // ebp
	let  input_byte_pos_final: number; // r10
	let  byte_final: number; // rbx
	let  brih_bytes: number; // er11
	let byte_tmp: number; // rdx
	let stream_len_needed: number; // r14
	let result: number; // rax
	let  inv_mask_out: number; // r8
	let qw70: number; // rcx
	let stream_compressed_size_new: number; // rdx
}