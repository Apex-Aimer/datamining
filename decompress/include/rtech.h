#include <cstdint>
#include <array>

// struct __declspec(align(8)) rpak_decomp_state
typedef struct
{
	uint64_t input_buf;
	uint64_t out;
	uint64_t mask;
	uint64_t out_mask;
	uint64_t file_len_total;
	uint64_t decompressed_size;
	uint64_t inv_mask_in;
	uint64_t inv_mask_out;
	unsigned int header_skip_bytes_bs;
	uint32_t dword44;
	uint64_t input_byte_pos;
	uint64_t decompressed_position;
	uint64_t len_needed;
	uint64_t byte;
	uint32_t byte_bit_offset;
	uint32_t dword6C;
	uint64_t qword70;
	uint64_t stream_compressed_size;
	uint64_t stream_decompressed_size;
} RpakDecompressionState;
// ASSERT_SIZE(rpak_decomp_state, 0x88);

namespace
{
	/*unk_141313180*/
	const unsigned char LUT_0[0x720] =
		{
			,
	};
}

class RTech
{
public:
	static uint64_t DecompressPakfileInit(std::int64_t param_buf, std::uint8_t *file_buf, std::int64_t file_size, std::int64_t off_no_header, std::int64_t header_size);
	static uint8_t DecompressPakfile(std::int64_t *param_buffer, std::uint64_t file_size, std::uint64_t buf_size);
	static uint64_t StringToGuid(const char *asset_name);
};
